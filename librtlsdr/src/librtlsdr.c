 ...
 * ported to ESP32 in 2025 by  Aleksey T <xtr@bk.ru>
 *
...
int rtlsdr_read_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
	...
	r = libusb_control_transfer(dev->devh, CTRL_IN, USB_B_REQUEST_GET_STATUS , addr, index, array, len, CTRL_TIMEOUT);
#if 1
	if (r < 0)
...
int rtlsdr_write_array(rtlsdr_dev_t *dev, uint8_t block, uint16_t addr, uint8_t *array, uint8_t len)
{
	...
	r = libusb_control_transfer(dev->devh, CTRL_OUT, USB_B_REQUEST_GET_STATUS , addr, index, array, len, CTRL_TIMEOUT);
#if 0
	if (r < 0)
		fprintf(stderr, "%s failed with %d\n", __FUNCTION__, r);
#endif
...
{
	ESP_LOGD(TAG,"rtlsdr_get_xtal_freq");
	if (!dev)
		return -1;
...
{
	ESP_LOGD("librtlsdr","rtlsdr_set_direct_sampling");
	int r = 0;
...
uint32_t rtlsdr_get_device_count(void)
{
	...
	int num_dev = info.num_devices;
	uint8_t *dev_addr_list;
	int num_dev_ret;
	if (num_dev==0){
		ESP_LOGE(TAG, "num_dev = 0");
		return 0;
	}

	dev_addr_list = malloc(num_dev);

	esp_err_t err = usb_host_device_addr_list_fill(num_dev, dev_addr_list, &num_dev_ret);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "dc: usb_host_device_addr_list_fill ERR, num_dev=%d", num_dev);
    } else {
		ESP_LOGV(TAG, "dc: usb_host_device_addr_list_fill ESP_OK num_dev_ret=%d", num_dev_ret);
    }
	
	for (i = 0; i < num_dev_ret; i++) {
		usb_device_handle_t dev_hdl;
		esp_err_t err = usb_host_device_open(uh_handle,dev_addr_list[i], &dev_hdl);
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "dc: usb_host_device_open ERR");
		} else {
			ESP_LOGD(TAG, "dc: usb_host_device_open ESP_OK");
		}
		const usb_device_desc_t *dd;
		usb_host_get_device_descriptor(dev_hdl, &dd);

		if (find_known_device(dd->idVendor, dd->idProduct))
			device_count++;
		usb_host_device_close(uh_handle,dev_hdl);			
	}
	free(dev_addr_list);
...
int rtlsdr_get_device_usb_strings(uint32_t index, char *manufact, char *product, char *serial)
{
	...
	esp_err_t err = usb_host_device_addr_list_fill(num_dev, dev_addr_list, &num_dev_ret);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "us: usb_host_device_addr_list_fill ERR, num_dev=%d", num_dev);
    } else {
		ESP_LOGV(TAG, "us: usb_host_device_addr_list_fill ESP_OK num_dev_ret=%d", num_dev_ret);
    }
	...
	for (i = 0; i < num_dev_ret; i++) {
		...
		esp_err_t err = usb_host_device_open(uh_handle,dev_addr_list[i], &dev_hdl);
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "us: usb_host_device_open ERR");
		} else {
			ESP_LOGD(TAG, "us: usb_host_device_open ESP_OK");
		}
		const usb_device_desc_t *dd;
		usb_host_get_device_descriptor(dev_hdl, &dd);
		...
		if (find_known_device(dd->idVendor, dd->idProduct)){
			...
			if (index == device_count-1){
				...
				getUsbDescString(dev_info.str_desc_manufacturer, manufact);
				getUsbDescString(dev_info.str_desc_product, product);
				getUsbDescString(dev_info.str_desc_serial_num, serial);
				usb_host_device_close(uh_handle,dev_hdl);							
				break;
			}
		}
...
const char *rtlsdr_get_device_name(uint32_t index)
{
	...
	esp_err_t err = usb_host_device_addr_list_fill(num_dev, dev_addr_list, &num_dev_ret);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "dn: usb_host_device_addr_list_fill ERR, num_dev=%d", num_dev);
    } else {
		ESP_LOGV(TAG, "dn: usb_host_device_addr_list_fill ESP_OK num_dev_ret=%d", num_dev_ret);
    }
	...
	for (i = 0; i < num_dev_ret; i++) {
		...
		esp_err_t err = usb_host_device_open(uh_handle,dev_addr_list[i], &dev_hdl);
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "dn:  usb_host_device_open ERR");
		} else {
			ESP_LOGD(TAG, "dn: usb_host_device_open ESP_OK");
		}
		const usb_device_desc_t *dd;
		usb_host_get_device_descriptor(dev_hdl, &dd);
...
int rtlsdr_open(rtlsdr_dev_t **out_dev, uint32_t index)
{
	...
	esp_err_t err = usb_host_device_addr_list_fill(num_dev, dev_addr_list, &num_dev_ret);
	if (err != ESP_OK) {
		ESP_LOGE(TAG, "ro: usb_host_device_addr_list_fill ERR, num_dev=%d", num_dev);
    } else {
		ESP_LOGV(TAG, "ro: usb_host_device_addr_list_fill ESP_OK num_dev_ret=%d", num_dev_ret);
    }
	
	//usb_device_handle_t dev_hdl;
	
	for (i = 0; i < num_dev_ret; i++) {
		device = dev_addr_list[i];	
		esp_err_t err = usb_host_device_open(uh_handle,device, &dev->devh);
		if (err != ESP_OK) {
			ESP_LOGE(TAG, "ro: usb_host_device_open ERR");
		} else {
			ESP_LOGD(TAG, "ro: usb_host_device_open ESP_OK");
		}
		const usb_device_desc_t *dd;
		usb_host_get_device_descriptor(dev->devh, &dd);
		...
		device = 0;
		usb_host_device_close(uh_handle,dev->devh);			
	}
	free(dev_addr_list);
	...
	r = usb_host_interface_claim(uh_handle, dev->devh, 0, 0);
	if (r < 0) {
		fprintf(stderr, "ro:  usb_claim_interface error %d\n", r);
		goto err;
	}
	ESP_LOGI("librtlsdr","ro:  usb_claim_interface OK");
	...
	ESP_LOGI("librtlsdr","ro:  init_baseband");
...
found:
	...
	ESP_LOGD(TAG,"tuner init");
	if (dev->tuner->init)
		r = dev->tuner->init(dev);
	ESP_LOGD(TAG,"tuner init end");	

	rtlsdr_set_i2c_repeater(dev, 0);

	*out_dev = dev;
	ESP_LOGD("librtlsdr","ro: end");
	return 0;
err:
	return r;
}
...
int rtlsdr_close(rtlsdr_dev_t *dev)
{
	...
	usb_host_interface_release(uh_handle,dev->devh,0);
	usb_host_device_close(uh_handle,dev->devh);			
	free(dev);
	ESP_LOGD("librtlsdr", "rtlsdr_close");
	return 0;
}
...
static void LIBUSB_CALL _libusb_callback(usb_transfer_t *xfer)
{
	...
	if (USB_TRANSFER_STATUS_COMPLETED == xfer->status) {
		...
		dev->xfer_errors = 0;
	} else if (USB_TRANSFER_STATUS_CANCELED != xfer->status) {
		if (USB_TRANSFER_STATUS_ERROR == xfer->status) 	dev->xfer_errors++;

		if (dev->xfer_errors >= dev->xfer_buf_num || USB_TRANSFER_STATUS_NO_DEVICE == xfer->status) {
			dev->dev_lost = 1;
...
static int _rtlsdr_alloc_async_buffers(rtlsdr_dev_t *dev)
{
	...
	if (!dev){
		ESP_LOGD(TAG, "alloc_async_buffers exit -1");
		return -1;
	}

	if (!dev->xfer) {
		dev->xfer = malloc(dev->xfer_buf_num * sizeof(usb_transfer_t *));

		for(i = 0; i < dev->xfer_buf_num; ++i){
			esp_err_t err;
			err =  usb_host_transfer_alloc(dev->xfer_buf_len, 0, &dev->xfer[i]);
			if (err != ESP_OK) {
				ESP_LOGE(TAG, "aab: usb_host_transfer_alloc ERR=%d", err);
				return -1;
			} else {
				ESP_LOGV(TAG, "aab: usb_host_transfer_alloc OK");
			}
			if (dev->xfer[i]->data_buffer_size != dev->xfer_buf_len){
				ESP_LOGE(TAG, "aab: data_buffer_size ERROR");
			}
...
{
	ESP_LOGD(TAG, "free_async_buffers");
	unsigned int i;

	if (!dev){
		ESP_LOGD(TAG, "free_async_buffers exit -1");
		return -1;
	}
	...
	ESP_LOGD(TAG, "free_async_buffers exit 0");	
	return 0;
}
...
{
	ESP_LOGD(TAG, "rtlsdr_read_async");
	unsigned int i;
	int r = 0;
	...
	if (!dev){
		ESP_LOGD(TAG, "rtlsdr_read_async exit -1 ");
		return -1;
	}	

	if (RTLSDR_INACTIVE != dev->async_status){
		ESP_LOGD(TAG, "rtlsdr_read_async exit -2");
		return -2;
	}	
	...
	if (buf_len > 0 && buf_len % 64 == 0) /* len must be multiple of 64 */
		dev->xfer_buf_len = buf_len;
	else
		dev->xfer_buf_len = DEFAULT_BUF_LENGTH;

    ESP_LOGD(TAG, "num_buf = %d, buf_len = %d", dev->xfer_buf_num, dev->xfer_buf_len);		
	r = _rtlsdr_alloc_async_buffers(dev);
	if (r!=0){
		ESP_LOGE(TAG, "ra: _rtlsdr_alloc_async_buffers ERR %d", r);
	...
	ESP_LOGD(TAG, "rtlsdr_read_async exit r");
	return r;
}

int rtlsdr_cancel_async(rtlsdr_dev_t *dev)
{
	ESP_LOGD(TAG, "cancel_async");
	if (!dev){
		ESP_LOGD(TAG, "cancel_async exit -1");
		return -1;
	}
	...
	if (RTLSDR_RUNNING == dev->async_status) {
		dev->async_status = RTLSDR_CANCELING;
		dev->async_cancel = 1;
		ESP_LOGD(TAG, "cancel_async exit 0");
		return 0;
	}
...
#endif
	ESP_LOGD(TAG, "cancel_async exit -2");
	return -2;
}
...
usb_transfer_t* getbuffer(rtlsdr_dev_t *dev, int i){
	return dev->xfer[i];
}