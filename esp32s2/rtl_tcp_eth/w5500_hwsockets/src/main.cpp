...
#define CONFIG_ETH_SPI_MISO_GPIO 13
#define CONFIG_ETH_SPI_MOSI_GPIO 11
#define CONFIG_ETH_SPI_SCLK_GPIO 12
#define CONFIG_ETH_SPI_CS0_GPIO   10
#define CONFIG_ETH_SPI_INT0_GPIO   4
#define CONFIG_ETH_SPI_POLLING0_MS_VAL  0
#define CONFIG_ETH_SPI_POLLING0_MS CONFIG_ETH_SPI_POLLING0_MS_VAL
#define CONFIG_ETH_SPI_PHY_RST0_GPIO -1
#define CONFIG_ETH_SPI_PHY_ADDR0  1
#define CONFIG_ETH_SPI_CLOCK_MHZ 40
	...
	while(1) {
		...
		while(curelem != 0) {
			...
			while(bytesleft > 0) {
				...
				if(r) {
					bytessent = send(s, (uint8_t*) &curelem->data[index],  bytesleft);
					bytesleft -= bytessent;
					index += bytessent;
				}
...
static void *command_worker(void *arg)
{
	...
	while(1) {
		...
		while(left >0) {
			...
			if(r) {
				received = recv(s, (uint8_t*) &cmd+(sizeof(cmd)-left), left);
				left -= received;
			}
...
//	struct linger ling = {1,0};
	...
	printf("xtrsdr/rtl_tcp_eth_w5500hw v1.0.0 start\n");  
	...
	spi_bus_initialize(CONFIG_ETH_SPI_HOST, &buscfg, SPI_DMA_CH_AUTO);
    ...
    gpio_config_t io_conf = {
        .pin_bit_mask = (1ULL << LED_GPIO) || (1ULL << CONFIG_ETH_SPI_CS0_GPIO) || ,
	...
	/* Create TCP socket*/
	listensocket = socket(0, Sn_MR_TCP, 1234,0);
	if (listensocket < 0)
	{
	...
	while(1) {
		...
		if( (ret = listen(listensocket)) != SOCK_OK)
    	{
			printf("Listen error\r\n");
			return;
		}			
...
/*		getnameinfo((struct sockaddr *)&remote, rlen,
		...
		r = send(s, (uint8_t*) &dongle_info, sizeof(dongle_info));
